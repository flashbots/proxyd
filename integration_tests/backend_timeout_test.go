...
func TestBackendSpecificTimeout(t *testing.T) {
	...
	t.Run("backend timeout fallback behavior", func(t *testing.T) {
		...
		// and less than 3.5 seconds (time to hit both backends plus a threshold)
	...
	t.Run("both backends timeout => 503 Service Unavailable", func(t *testing.T) {
		...
		// and less than 8.5 seconds (time to hit both backends plus a threshold)
		require.Less(t, elapsed, 8500*time.Millisecond)
